...
# You should have received a copy of Fluky license with
...
function ubuntu_18_04 {
    ...
    # welcome message
    echo ""
    echo -e "${LIGHTCYAN}WELCOME TO THE FLUKY INSTALLATION!${NOCOLOR}"
    echo ""
    ...
    # create database
    mysql -e "create database fluky;"

    mysql -e "alter database fluky character set utf8 collate utf8_general_ci;"
...
DocumentRoot /var/www/html/fluky/public
...
" > /etc/turnserver.conf
    ...
    mkdir /var/www/html/fluky
    chown $USER:www-data /var/www/html/fluky

    apt install unzip -y

    unzip fluky.zip -d /var/www/html/fluky

    cd /var/www/html/fluky
...
" > /etc/coturn/turnserver.conf
    ...
    # setting up installation folder
    mkdir /var/www/html/fluky
    chown $USER:apache /var/www/html/fluky
...
1) This script and the ${LIGHTRED}fluky.zip${NOCOLOR} file must be in the same directory
2) The zip file must be named as ${LIGHTRED}fluky.zip${NOCOLOR} (case sensitive)"
            echo ""
